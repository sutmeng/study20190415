...
import com.boot.security.server.utils.PageData;
import com.boot.security.server.utils.Tools;
...
public class Page {
    ...
    private PageData pd = new PageData();//定义存储各种值的对象
    private String orderBy=null;//定义排序，这里是字符串，就是select语句order by 后面的部分，多个排序间以逗号分隔


    public Page(){
        try {
            this.showCount =Const.PageSize; //Integer.parseInt(Tools.readTxtFile(Const.PAGE));
        } catch (Exception e) {
            this.showCount = 10;
        }
    }
